...
predict.sparse <- function(sp, new.data, delta = 24 ) {
  ...
  # we don't know the variance of the part to be estimated, 
  # but that can be assumed to be the same as that of the part we do have.
  cm <- apply(X,2,mean)
  cv <- apply(X,2,sd)
  for (j in 1:N) {
    X[, j] <- (X[,j] - cm[j]) / (2 * cv[j])
  ...
  for (j in 1:N) {
    Xj <- X1[, j]
    for (i in 1:m) {
      Xji <- Xj[((i - 1) * delta + 1):(i * delta)]
      X1[((i - 1) * delta + 1):(i * delta), j] <- Xji
    }
  }
  return(X1)
}
